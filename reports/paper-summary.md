...
در این بخش، مدل کلاسیک پذیرش دانشگاهی که توسط گیل و شپلی ارائه شده است بررسی می‌شود. در این حالت از محدودیت‌هایی مثل مساوی شدن نمرات دانشجوها و سهمیه‌های دانشگاهی صرف‌نظر شده و برای آن‌ها قیدی تعریف نمی‌کنیم.
ابتدا مفاهیم پایه و نمادگذاری‌ها معرفی می‌شوند، سپس چهار فرمول‌بندی مختلف برنامه‌ریزی خطی با اعداد صحیح (IP) برای این مدل ارائه و با یکدیگر مقایسه می‌شوند.

### ۲.۱. تعاریف و مفاهیم جدید
...
پیاده‌سازی در مقاله اصلی با استفاده از نرم‌افزار AMPL و سالور Gurobi انجام شده است.

نتایج عملکرد در جدول ۲ مقاله خلاصه شده است:

![Paper Table 2](photos/Comb-Opt-Project-Report-1.png)

## بخش ۳: مدل‌های با رتبه‌های مساوی (Models with Ties)

در این بخش، حالت مساوی شدن رتبه‌های دانشجوها را به فرمول‌بندی‌های قبلی اضافه می‌کنیم. رتبه‌های مساوی زمانی رخ می‌دهند که دو یا چند دانشجو نمره یکسانی در یک برنامه داشته باشند.

در ایرلند، این رتبه‌ها با قرعه‌کشی شکسته می‌شود، اما در کشورهایی مانند مجارستان و شیلی، سیاست رفتار برابر (Equal Treatment Policy) اعمال می‌شود، یعنی به این معنی که دانشجویان با نمره مساوی یا همگی پذیرفته می‌شوند یا همگی رد می‌شوند.

### ۳.۱. تعاریف و مفاهیم اولیه

در این بخش، دو سیاست متفاوت برای مواجهه با رتبه‌های مساوی معرفی می‌شوند:

1. سیاست محدودکننده مجارستان (Hungarian Restrictive Policy)
	- ظرفیت دانشگاه‌ها هرگز نقض نمی‌شود.
	- اگر آخرین گروه از دانشجویان با نمره مساوی در ظرفیت باقیمانده نگنجند، همه رد می‌شوند.
	- این سیاست با نام H-stability نیز شناخته می‌شود.
2. سیاست انعطاف‌پذیر شیلی (Chilean Permissive Policy)
	- آخرین گروه از دانشجویان با نمره مساوی همگی پذیرفته می‌شوند، حتی اگر این امر منجر به نقض ظرفیت شود.
	- این سیاست با نام L-stability نیز شناخته می‌شود.

#### ۳.۱.۱. تابع انتخاب (Choice Function)
...
### ۳.۲. فرمول‌بندی‌های برنامه‌ریزی خطی با اعداد صحیح (IP Formulations)

در این بخش، ابتدا فرمول‌بندی‌های قبلی که برای حالت رتبه‌های مساوی کار می‌کنند معرفی می‌شوند، سپس فرمول‌بندی‌های جدید ارائه می‌گردند.

#### ۳.۲.۱. فرمول‌بندی‌های قبلی که برای رتبه‌های مساوی کار می‌کنند
...
- **روش MIN-CUT**: با قیدهای (۱)، (۲)، (۵)، (۶) و تابع هدف (۹)
- **روش MSMR-CUT**: با قیدهای (۱)، (۲)، (۵)، (۶) و تابع هدف (۱۰)
- **روش MIN-BIN-CUT**: با قیدهای (۱)، (۲)، (۱۱)، (۱۲)، (۱۳) و تابع هدف (۱۵)
- **روش MSMR-BIN-CUT**: با قیدهای (۱)، (۲)، (۱۱)، (۱۲)، (۱۳) و تابع هدف (۱۰)
- **روش MSMR-EF**: با قیدهای (۱)، (۲)، (۱۶) و تابع هدف (۱۰) (اما این مدل را بیش‌تر از این بررسی نمی‌کنیم، زیرا زمان محاسبه‌ی آن در شبیه‌سازی‌ها خیلی زیاد بود.)

#### ۳.۲.۲. فرمول‌بندی جدید: SO-H-NW-CUT
...
#### ۳.۲.۳. فرمول‌بندی دودویی: SO-H-NW-BIN-CUT
...
مانند بخش قبلی، نویسنده‌ها برای ارزیابی عملکرد این فرمول‌بندی‌ها از داده‌های واقعی مجارستان در سال ۲۰۰۸ استفاده کردند. در این بخش، رتبه‌های مساوی نیز مد نظر گرفته‌شده، اما در بخش سهمیه‌ها، فقط سهمیه‌های دانشکده‌ای لحاظ شده است. برای هر برنامه، فقط بالاترین رتبه درخواست هر دانشجو نگه‌داری شده است.

نتایج عملکرد در جدول ۳ مقاله خلاصه شده است:

![Paper Table 3](photos/Comb-Opt-Project-Report-2.png)

**تحلیل نتایج:**

1. **برتری فرمول‌بندی دودویی:** بهترین فرمول‌بندی دودویی (SO-H-NW-BIN-CUT)، عملکرد بسیار بهتری نسبت به بهترین فرمول‌بندی پیوسته (SO-H-NW-CUT) دارد. این برتری در مقایسه با مدل پایه (که نسخه پیوسته اندکی بهتر بود) قابل توجه است و نشان می‌دهد که در حضور رتبه‌های مساوی، متغیرهای دودویی کارایی بالاتری دارند.
2. **تاثیر قیدهای عدم اسراف مستقیم:** فرمول‌بندی‌های SO-H-NW-CUT و SO-H-NW-BIN-CUT که مستقیماً شرط عدم اسراف را اعمال می‌کنند، در مقایسه با فرمول‌بندی‌های مبتنی بر توابع هدف (MIN و MSMR) عملکرد بهتری دارند. این نشان می‌دهد که اعمال مستقیم قیدها به جای اتکا به تابع هدف، در این مسئله کارآمدتر است.
3. **پیچیدگی محاسباتی:** فرمول‌بندی SO-H-NW-BIN-CUT با وجود تعداد متغیرها و قیدهای بیشتر زمان حل بسیار کمتری دارد. این نشان می‌دهد که ساختار قیدهای دودویی باعث ایجاد یک مسئله با خواص بهتر برای سالورهای IP می‌شود.

## بخش ۴: مقایسه سیاست‌ها برای رتبه‌های مساوی

در این بخش ابتدا فرمول‌بندی برای سیاست شیلی (انعطاف‌پذیر و پذیرش دانشجویان با رتبه برابر) ارائه می‌شود و در نهایت نتایج سه فرمول‌بندی (برای سیاست‌های مختلف شیلی، ایرلند و مجارستان) مقایسه می‌شود.

### ۴.۱. فرمول‌بندی IP برای سیاست شیلی

در سیاست شیلی، دانشجویانی با نمره مساوی که برای آخرین جایگاه‌های خالی یک موقعیت (که کم‌تر از تعدادشان است) رقابت می‌کنند، همگی پذیرفته می‌شوند، حتی اگر این امر منجر به نقض ظرفیت شود. به عبارت دیگر، اگر نمره برش یک واحد افزایش یابد، تعداد دانشجویان پذیرفته‌شده به شدت کمتر از ظرفیت خواهد شد.

برای مدل‌سازی این سیاست، از فرمول‌بندی مشابه SO-H-NW-CUT استفاده می‌شود، اما با متغیرهای جدید و قیدهای متفاوت.

#### ۴.۱.۱. فرمول‌بندی نمره برش پیوسته - سیاست شیلی (SO-C-NW-CUT)
...
#### ۴.۱.۲. فرمول‌بندی نمره برش دودویی - سیاست شیلی (SO-C-NW-BIN-CUT)
...
### ۴.۲. نتایج محاسباتی و مقایسه سیاست‌ها

مشابه دو بخش قبلی، شبیه‌سازی‌ها روی داده‌های سال ۲۰۰۸ انجام شده و رتبه‌های مساوی و فقط سهمیه‌های دانشکده‌ای در نظر گرفته شده است.
برای شبیه‌سازی سیاست ایرلند، ابتدا روی داده‌های ورودی پیش‌پردازش انجام شده و رتبه‌های مساوی به‌صورت تصادفی شکسته شده اند، سپس مدل‌های کلاسیک (بدون رتبه مساوی) حل شده‌اند.
برای یافتن جواب بدترین برای دانشجو (Student-Pessimal)، به جای بیشینه‌سازی تابع هدف (۱۰)، آن را کمینه‌سازی می‌کنیم، که معادل انتخاب بدترین تطابق پایدار برای دانشجویان است.

نتایج در جدول ۴ مقاله خلاصه شده است:

![Paper Table 4](photos/Comb-Opt-Project-Report-3.png)

این جدول چهار معیار اصلی را برای هر سه سیاست مقایسه می‌کند:

1. اندازه تطابق (Size of Matching): تعداد کل دانشجویان پذیرفته‌شده
2. میانگین رتبه (Average Rank): میانگین رتبه‌های پذیرش دانشجویان (هرچه کمتر، بهتر)
3. میانگین نمرات برش (Average Cutoffs): میانگین نمرات برش دانشگاه‌ها (هرچه کمتر، برای دانشجویان بهتر است)
4. تعداد ردشد‌ه‌ها (Number of Rejections): تعداد کل ردشدگی‌ها

نتایج مقاله اصلی در این جدول، به طور کامل قضایای Biro & Kiselgof (2015) که در مقدمه آورده شد را تایید می‌کند:
> **نمرات برش در مجارستان ≥ نمرات برش در ایرلند ≥ نمرات برش در شیلی**

این رابطه به صورت زیر در نتایج مشاهده می‌شود:
- میانگین نمرات برش در مجارستان: $58.3931$
- میانگین نمرات برش در ایرلند: $58.2090$
- میانگین نمرات برش در شیلی: $57.2000$

در در واقع:
- دانشجویان در مجارستان بدترین تطابق ممکن را دریافت می‌کنند (بالاترین نمرات برش، بالاترین میانگین رتبه، بیشترین تعداد ردشدگی)
- دانشجویان در ایرلند تطابق بهتری نسبت به مجارستان دریافت می‌کنند
- دانشجویان در شیلی بهترین تطابق ممکن را دریافت می‌کنند (پایین‌ترین نمرات برش، پایین‌ترین میانگین رتبه، کمترین تعداد ردشدگی)

نکات جالب دیگری که در این مقاله ذکر شده است:
- برای سیاست‌های مجارستان و شیلی، هیچ تفاوتی بین جواب‌های بهینه دانشجو (Student-Optimal) و بدترین برای دانشجو (Student-Pessimal) مشاهده نشد. این یعنی جواب‌های پایدار برای این دو سیاست یکتا هستند.
- برای سیاست ایرلند، تفاوت بین جواب‌های بهینه و بدترین برای دانشجو بسیار ناچیز است.

این یافته‌ها با نتایج قبلی در بازارهای بزرگ (مانند NRMP در مقاله Roth & Peranson, 1999) همخوانی دارد و نشان می‌دهد که در بازارهای بزرگ، تفاوت بین جواب‌های بهینه و بدترین برای دانشجو معمولاً ناچیز است.
